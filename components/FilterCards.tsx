...
export default function FilterCards() {
  ...
  const [kicksFilter, setKicksFilter] =
    useState<{ drop: ProductCardType[] }[]>();
  const [isFilterCard, setIsFilterCard] = useState<boolean>(false);
  const filterKicks = useCallback(
    (kicks: ProductCardType[]) => {
  ...
  const handlePrevious = () =>
    previousRef.current && previousRef.current.click();
  const submitFilter = (data: FilterValues) => {
    const filteredData = kicks?.filter((kick) => {
      return (
        (!data?.colors.length ||
          kick.colors?.some((color) => data.colors?.includes(color))) &&
        (!data?.genders.length ||
          kick.gender?.some((gender) => data.genders?.includes(gender))) &&
        (!data.sizes.length ||
          kick.sizes?.some((size) => data.sizes.includes(size))) &&
        (!data.price.length ||
          (kick.price &&
            kick.price >= data.price[0] &&
            kick.price <= data.price[1])) &&
        (!data.categories.length ||
          kick.types?.some((type) => data.categories.includes(type)))
      );
    });
    if (filteredData) {
                ...
                {kicksFilter?.map((box, index) => (
                  ...
                  >
                    {box?.drop?.map((kick, indx) => (
                      <ProductCard key={indx} kick={kick} />
                    ))}
                  </CarouselItem>
                ))}
              </CarouselContent>
